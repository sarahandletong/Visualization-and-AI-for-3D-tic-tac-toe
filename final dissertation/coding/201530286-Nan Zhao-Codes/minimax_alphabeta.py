...
class Connect3D(object):
    ...
    def __init__(self, grid_size=grid_size_recommended):
        ...
        pygame.init()
        pygame.display.set_caption("Minimax algorithm with alpha-beta pruning")
    ...
    def show_score(self, digits=False, marker='/'):
        ...
        return 'Player A (Human)|| {a}  Player B (AI)|| {b}'.format(a=multiply_value * (self.current_points['A']),
                                                                          b=multiply_value * self.current_points['B'])
...
def get_max_dict_keys(x):
    if x:
        print(f"player：{x['X'] + x['O']}  computer：{x['Y'] + x['W']}")
        if x['X'] + x['O'] > x['Y'] + x['W']:
            print("player win")
        elif x['X'] + x['O'] < x['Y'] + x['W']:
            print("computer win")
        else:
            print("the tie")
...
class SimpleC3DAI(object):
    ...
    def minimax(self, Board, maximizingPlayer, depth, count):
        ...
        if maximizingPlayer:
            ...
            for move in moves_list:
                ...
                the_score, the_move, count = self.minimax(new_board, False, depth - 1, count + 1)
        ...
        # minimzing player
            ...
            for move in moves_list:
                new_board = deepcopy(Board)
                new_board.play_legal_move(move[0], move[1], player, opp, flip=True)
                the_score, the_move, count = self.minimax(new_board, True, depth - 1, count + 1)
    ...
    def alphabeta(self, Board, maximizingPlayer, depth, count, alpha, beta):
        # maximizing player has 'B' and minimizing 'W'
        if maximizingPlayer:
            player, opp = Board.player, Board.opp
        else:
        ...
        if maximizingPlayer:
            ...
            for move in moves_list:
                new_board = deepcopy(Board)
                new_board.play_legal_move(move[0], move[1], player, opp, flip=True)
                the_score, the_move, count = self.alphabeta(new_board, False, depth - 1, count + 1, alpha, beta)
                if (the_score > alpha):
                    alpha = the_score
                    best_move = move
                if beta <= alpha: break
        ...
        # minimzing player
            ...
            for move in moves_list:
                new_board = deepcopy(Board)
                new_board.play_legal_move(move[0], move[1], player, opp, flip=True)
                the_score, the_move, count = self.alphabeta(new_board, True, depth - 1, count + 1, alpha, beta)
                if (the_score < beta):
                    beta = the_score
                    best_move = move
                if beta <= alpha: break
